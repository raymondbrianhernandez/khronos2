<div style="text-align: left; padding: 20px; margin: auto;">
    ...
    $assignee         = mysqli_real_escape_string ( $con, $assignee );
    
    $result_asssignee = mysqli_query ( $con, "SELECT * FROM `assignments` WHERE assignee = '$assignee'" );
    $result_assistant = mysqli_query ( $con, "SELECT * FROM `assignments` WHERE assistant = '$assignee'" );
    ...
    if ( $result_asssignee ) {
        ...
        echo '<span style="font-size: 1.1rem"><b>Upcoming part(s):</b></span><br>';
        foreach ( $results as $i => $result ) {
            echo "<b><i>" . ( $i + 1 ) . ". Week of " . $result['week'] . "</i></b><br>";
            echo $result['part'] . "<br><br>";
        }
        if ( empty ( $results ) ) {
            echo "No assignments yet.<br>";
        }
    } else {
    ...
    if ( $result_assistant ) {
        ...
        echo '<hr><span style="font-size: 1.1rem"><b>Upcoming part(s) as an assistant:</b></span><br>';
        foreach ( $results as $i => $result ) {
            echo "<b><i>" . ( $i + 1 ) . ". Week of " . $result['week'] . "</i></b><br>";
            echo $result['part'] . "<br><br>";
        }
